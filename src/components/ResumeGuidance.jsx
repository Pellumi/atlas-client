...
const ResumeGuidance = () => {
      ...
      try {
        ...
        const convs = Array.isArray(convData)
          ? convData.map(({ _id, title, created_at }) => ({
              ...
              type: title?.toLowerCase().includes("career") ? "career" : "resume",
            }))
          : []
  ...
  const startNewChat = (type = "resume") => {
    ...
    setConversationTitle(type === "career" ? "New career guidance" : "New resume guidance")
  }
  ...
  const initNewChat = async (question) => {
    ...
    try {
      ...
      const resp = await resumeGuidanceService.createConversation(question.trim(), resumeFile, useProfileResume)
      ...
      setConversationTitle(resp.conversation_title || question.trim().substring(0, 30))
      ...
      console.error(err)
      setError(`Failed to create new ${conversationType} guidance conversation`)
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const selectConversation = async (id) => {
    ...
    try {
      ...
      const type = conv?.type || (conv?.title?.toLowerCase().includes("career") ? "career" : "resume")
      setConversationType(type)
      ...
      setConversationTitle(sidebarConv?.title || convData.conversation_title || "Guidance")
  ...
  const sendMessage = async (convId, content) => {
    ...
    try {
      const resp = await resumeGuidanceService.sendMessage(convId, content, resumeFile, useProfileResume)
  ...
  const giveFeedback = async (id, positive) => {
    try {
      await resumeGuidanceService.provideFeedback(id, positive ? "positive" : "negative")
  ...
  return (
    <div className="resume-guidance-container">
      {/* Sidebar */}
      <div className={`resume-guidance-sidebar ${showSidebar ? "open" : "closed"}`}>
        <div className="sidebar-header">
          <div>
            <h2>Guidance Sessions</h2>
            <button onClick={toggleSidebar} className="toggle-sidebar-button">
              <span className={`icon-arrow-left ${!showSidebar ? "rotate-180" : ""}`} />
            </button>
          </div>
            ...
            sortedConversations.map((c) => (
              ...
              >
                <div
                  ...
                  style={{ backgroundColor: c.type === "career" ? "#10b981" : "#3b82f6" }}
                >
                  <span className={c.type === "career" ? "icon-briefcase" : "icon-file-text"} />
                </div>
                <div className="conversation-details">
                  <div className="conversation-title">{c.title}</div>
                  <div className="conversation-preview">{c.preview || ""}</div>
                  <div className="conversation-date">{convertToReadableDateTime(c.date)}</div>
                </div>
              </div>
            ))
          )}
      ...
      <div className="resume-guidance-main">
        <header className="resume-guidance-header">
          <div className="header-left">
            ...
            <h1 className="resume-guidance-title">
              ...
              {conversationType === "career" && activeTab === "chat" && <span className="career-badge">Career</span>}
            </h1>
          </div>
          <div className="header-right">
            {activeTab === "chat" && careerProfile && conversationType === "resume" && (
              <button
                className={`use-profile-button ${useProfileResume ? "active" : ""}`}
        ...
        {activeTab === "chat" ? (
          <>
            <div className="messages-container">
              {isLoading && !messages.length ? (
                ...
                <div className="empty-guidance">
                  <h3>Get personalized {conversationType === "career" ? "career" : "resume"} guidance</h3>
                  <div className="resume-guidance-intro">
                    {conversationType === "career" ? (
                      <p>Ask questions about your career path, skills development, or job search strategies.</p>
                    ) : (
                      <>
                        <p>Upload your resume for detailed feedback or ask specific questions about resume writing.</p>
                        {careerProfile && (
                          <div className="profile-resume-notice">
                            <p>
                              <span className="icon-info" /> You have a resume in your career profile. You can use it by
                              toggling "Use Profile Resume" above.
                            </p>
                          </div>
                        )}
                      </>
                  ...
                  <div className="suggested-questions">
                    {conversationType === "career"
                      ? [
                          ...
                          <button key={q} onClick={() => handleSuggestedQuestion(q)} className="suggested-question">
                            {q}
                          </button>
                        ))
                      : [
                ...
                messages.map((msg) => (
                  <div key={msg.id} className={`message ${msg.role}-message ${msg.pending ? "pending" : ""}`}>
                      ...
                      <div className="message-content">
                        {msg.hasFile && (
                          <div className="file-attachment">
                            ...
                            <span>{useProfileResume ? "Using profile resume" : "Resume uploaded"}</span>
                          </div>
                        )}
                        <ReactMarkdown>{msg.pending ? "Sending..." : msg.content}</ReactMarkdown>
                      </div>
                        ...
                        <>
                          ...
                          {msg.resumeRetouch && (
                            <div className="resume-retouch">
                              ...
                              <div className="retouch-content">
                                <ReactMarkdown>{msg.resumeRetouch}</ReactMarkdown>
                              </div>
                            </div>
                          )}
                          ...
                          <div className="feedback-container">
                            <div className="message-feedback">
                              <button
                                className={`feedback-button ${feedbackGiven[msg.id] === "positive" ? "active" : ""}`}
                              ...
                              <button
                                className={`feedback-button ${feedbackGiven[msg.id] === "negative" ? "active" : ""}`}
            ...
            <form onSubmit={handleSubmit} className="resume-guidance-input-container">
              <div className="resume-guidance-input-wrapper">
                ...
                {resumeFile && (
                  <div className="resume-file-preview">
                    ...
                    <button type="button" className="remove-file-button" onClick={() => setResumeFile(null)}>
          ...
          <CareerProfileView profile={careerProfile} onUpdate={handleProfileUpdate} isLoading={isLoading} />
        )}
      </div>
    </div>
  )
...
const CareerProfileView = ({ profile, onUpdate, isLoading }) => {
  ...
  console.log(formData);
    ...
    Object.entries(formData).forEach(([key, value]) => {
      if (value !== null && value !== undefined) {
        submitData.append(key, typeof value === "number" ? value.toString() : value)
      }
    })
  ...
  if (isLoading) {
    return <div className="loading-profile">Loading your career profile...</div>
  }
  ...
  if (!profile && !isEditing) {
    return (
      <div className="empty-profile">
        ...
        <p>Create your career profile to enhance your resume guidance experience.</p>
        <button className="create-profile-button" onClick={() => setIsEditing(true)}>
          Create Profile
        </button>
      </div>
    )
    ...
    <div className="career-profile-container">
      {isEditing ? (
        <form onSubmit={handleSubmit} className="profile-edit-form">
          ...
          <div className="form-group">
            <label htmlFor="education_duration">Education Duration (years)</label>
            <input
              type="number"
          ...
          <div className="form-group">
            ...
            <div className="file-input-wrapper">
              <input type="file" id="resume-file" name="file" onChange={handleFileChange} accept=".pdf,.doc,.docx" />
              {formData.file && <span className="file-selected">{formData.file.name}</span>}
              {!formData.file && profile?.resume_url && (
                <span className="current-file">Current: {profile.resume_filename || "resume.pdf"}</span>
              )}
            </div>
          </div>

          <div className="form-actions">
            <button type="button" className="cancel-button" onClick={() => setIsEditing(false)} disabled={isSaving}>
              Cancel
            </button>
        ...
        <div className="profile-view">
          ...
          <div className="profile-section">
            ...
            <p>{profile.education_duration ? `${profile.education_duration} years` : "Not specified"}</p>
          </div>
          ...
          <div className="profile-section">
            ...
            <div className="profile-text-content">
              {Array.isArray(profile.current_skills) && profile.current_skills.length > 0 ? (
                <ReactMarkdown>{profile.current_skills.join("\n")}</ReactMarkdown>
              ) : profile.current_skills ? (
